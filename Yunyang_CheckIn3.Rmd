...
```{r, message=FALSE}
data_logis <- data %>% dplyr::select(-depress) %>% drop_na()
...
```{r}
# anova(logis2,logis1,test="Chisq")$`Pr(>Chi)` # p<<0.05
# anova(logis3,logis1,test="Chisq")$`Pr(>Chi)` # p<<0.05
# anova(logis4,logis1,test="Chisq")$`Pr(>Chi)` # p<<0.05
# anova(logis5,logis1,test="Chisq")$`Pr(>Chi)` # p<<0.05
# anova(logis6,logis1,test="Chisq")$`Pr(>Chi)` # p<<0.05
# anova(logis7,logis1,test="Chisq")$`Pr(>Chi)` # p<<0.05
# 
# # logis1 based on LRT
```
...
# OR
# logis1, logis2, logis3, logis5 based on number of significant covariates
```
...
# OR
# logis3, logis2, logis6 based on AIC
# logis6, logis4, logis3 based on BIC
# logis1, logis2, logis3, logis4 based on AUC 

# stepModel <- step(logis1, direction=c("both"))
# sexcat + racecat + healthcat + esupport based on stepwise selection
# OR
# AGE + sexcat + racecat + healthcat + esupport + incomecat based on stepwise selection
...
data_multi <- data %>% dplyr::select(-anxiety) %>% drop_na()
# data_multi <- upsample(data_multi, "depress")
...
```{r}
# anova(multi2,multi1,test="Chisq")$`Pr(Chi)` # p<<0.05
# anova(multi3,multi1,test="Chisq")$`Pr(Chi)` # p<<0.05
# anova(multi4,multi1,test="Chisq")$`Pr(Chi)` # p<<0.05
# anova(multi5,multi1,test="Chisq")$`Pr(Chi)` # p<<0.05
# anova(multi6,multi1,test="Chisq")$`Pr(Chi)` # p<<0.05
# anova(multi7,multi1,test="Chisq")$`Pr(Chi)` # p<<0.05
# 
# # multi1 based on LRT
```
...
# OR
# multi1, multi2, multi5 based on AIC
# multi5, multi1 based on BIC
```
